...
def output_jsonlines_results(base_dir):
    ...
    for result in possible_files:
        ...
        output = json.dumps({exp_option: {"mean": np.mean(rollout_loss),
                                          "std": np.std(rollout_loss)}}, ensure_ascii=False)
...
def output_txt_results(base_dir, std=True):
    ...
    for result in possible_files:
        ...
        if std:
            results[LAMBDA][dm_model][policy_network] = "{:.3f}±{:.3f}".format(np.mean(rollout_loss), np.std(rollout_loss))
        else:
            results[LAMBDA][dm_model][policy_network] = "{:.3f}".format(np.mean(rollout_loss))
