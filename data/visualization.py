...
class CachedPlotter(object):
    ...
    def plot_with_errorbar(self, handle_name, xs, ys, yerr, *args, **kwargs):
        ...
        bars[0].set_segments(
            [
                np.array([[x, yt], [x, yb]])
                for x, yt, yb in zip(xs, ys + yerr, ys - yerr)
            ]
        )
    ...
    def scatter(self, handle_name, xs, ys, *args, **kwargs):
        ...
        xys = np.hstack(
            (np.atleast_1d(xs)[:, np.newaxis], np.atleast_1d(ys)[:, np.newaxis])
        )
        h.set_offsets(xys)
        self.ax.draw_artist(h)
...
def VStack(*imgs, align="center"):
    ...
    if align == "center":
        for img in imgs:
            ...
            imgs_padded.append(
                cv2.copyMakeBorder(img, 0, 0, left, right, cv2.BORDER_CONSTANT)
            )
...
def HStack(*imgs, align="center"):
    ...
    if align == "center":
        for img in imgs:
            ...
            imgs_padded.append(
                cv2.copyMakeBorder(img, top, bottom, 0, 0, cv2.BORDER_CONSTANT)
            )
...
class CartpoleVisualizer(object):
    ...
    def _draw_cartpole_helper(self, name, x, theta, alpha):
        ...
        self.plotter.rectangle(
            "%s-cart" % name, xy, self.cart_width, self.cart_height, alpha=alpha
        )
        ...
        self.plotter.line_collection(
            "%s-pole" % name,
            [[(x, 0), (x2, y2)]],
            linewidths=[self.pole_thickness],
            alpha=alpha,
        )
...
class Visualizer(object):
    ...
    def set_gp_delta_state_trajectory(self, ts, traj, variances):
        self.delta_state_trajs["gp"] = np.concatenate(
            [ts[:, None], traj, variances], axis=1
        )
    ...
    def draw(self, redraw=False):
        ...
        gt_cartpole = cv2.cvtColor(
            self.cartpole_vis_gt.draw_cartpole("", x, theta), cv2.COLOR_RGB2BGR
        )
        ...
        gp_cartpole = cv2.cvtColor(
            self.cartpole_vis_gp.draw_cartpole_batch(names, xs, thetas, alphas),
            cv2.COLOR_RGB2BGR,
        )
        ...
        for idx, data in enumerate((dxs, ddxs, dthetas, ddthetas)):
            gt_handles.append(
                self.plotters[idx].plot("gt", ts, data, "g", linewidth=1, alpha=0.8)
            )
        ...
        (
            ...
            dtheta_var,
            dx_var,
        ) = zip(*self.delta_state_trajs["gp"])
        vars = dx_var, ddx_var, dtheta_var, ddtheta_var
        gp_handles = []
        for idx, data in enumerate((dxs, ddxs, dthetas, ddthetas)):
            gp_handles.append(
                self.plotters[idx].plot_with_errorbar(
                    "gp",
                    ts,
                    data,
                    np.sqrt(vars[idx]) * 3,
        ...
        text_info = self.info_panel.draw_text(
            "info", 0.0, 1.0, self.info_text, fontsize=12, verticalalignment="top"
        )
